...
#ifdef USE_ISHAPE
    extern IShapeFactory *g_IShapeFactory;
#endif
...
void CCADView::OnDraw(CDC* pDC)
{
    ...
    */
...
#ifdef USE_ISHAPE
    CMyList<IShape> &ShapeList = pDoc->m_ShapeList;
#else
    ...
    {
    #ifdef USE_ISHAPE
        IShape *pmyShape = ShapeList.GetNext(pos);
    #else
...
{   
    ...
    IShape *pOldState = m_pSelectShape->CopySelf();

    m_pSelectShape->SetPosition(point);

    IShape *pNewState = m_pSelectShape->CopySelf();
...
void CCADView::OnLButtonDown(UINT nFlags, CPoint point) 
{
    //
#ifdef USE_ISHAPE
    m_pMyShapeFactory = g_IShapeFactory;
#endif
...
BOOL CCADView::OnCommand(WPARAM wParam, LPARAM lParam) 
{
