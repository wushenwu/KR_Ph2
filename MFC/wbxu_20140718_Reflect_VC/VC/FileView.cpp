// FileView.cpp : implementation file
//

#include "stdafx.h"
#include "VC.h"
#include "FileView.h"

#ifdef _DEBUG
#define new DEBUG_NEW
#undef THIS_FILE
static char THIS_FILE[] = __FILE__;
#endif

/////////////////////////////////////////////////////////////////////////////
// CFileView dialog


CFileView::CFileView(CWnd* pParent /*=NULL*/)
	: CDialog(CFileView::IDD, pParent)
{
	//{{AFX_DATA_INIT(CFileView)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
}


void CFileView::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CFileView)
	DDX_Control(pDX, IDC_FILETREE, m_FileTreeCtrl);
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CFileView, CDialog)
	//{{AFX_MSG_MAP(CFileView)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CFileView message handlers

BOOL CFileView::OnInitDialog() 
{
	CDialog::OnInitDialog();

    // TODO: Add extra initialization here
    if (m_ImgList != NULL)    //这样避免图片的重复加载
    {
        return FALSE;
    }
    
    //设置TreeView Style
    LONG lOld = GetWindowLong(GetSafeHwnd(), GWL_STYLE);
    SetWindowLong(GetSafeHwnd(), 
        GWL_STYLE, 
        lOld | TVS_HASBUTTONS | TVS_HASLINES | TVS_LINESATROOT
        );
    
    //加载图片列表
    m_ImgList.Create(16, 16, ILC_COLOR32 | ILC_MASK, 0, 5);
    m_ImgList.Add(LoadIcon(AfxGetInstanceHandle(), 
        MAKEINTRESOURCE(IDI_FILES)));
    m_ImgList.Add(LoadIcon(AfxGetInstanceHandle(), 
        MAKEINTRESOURCE(IDI_CLSVIEWCLASSES)));
    m_ImgList.Add(LoadIcon(AfxGetInstanceHandle(), 
        MAKEINTRESOURCE(IDI_CLSVIEWFOLDER)));
    m_ImgList.Add(LoadIcon(AfxGetInstanceHandle(), 
        MAKEINTRESOURCE(IDI_FOLDEROPEN)));
    m_ImgList.Add(LoadIcon(AfxGetInstanceHandle(), 
        MAKEINTRESOURCE(IDI_FILE)));

    
    //设置图片列表
    CTreeCtrl *pCtrl = (CTreeCtrl *)GetDlgItem(IDC_FILETREE);
    pCtrl->SetImageList(&m_ImgList, TVSIL_NORMAL );
    
    //添加item
    HTREEITEM hParent = pCtrl->InsertItem("Workspace 'wbxu_20140718'", 0, 0);
        HTREEITEM hSub = pCtrl->InsertItem("Reflect files", 1, 1, hParent);
            hSub = pCtrl->InsertItem("Source Files", 2, 3, hSub);
            pCtrl->InsertItem("MyBTN.cpp", 4, 4, hSub);

    hSub = pCtrl->InsertItem("VC files", 1, 1, hParent);
        hSub   = pCtrl->InsertItem("Source Files", 2, 3, hSub);
        pCtrl->InsertItem("ChildFrm.cpp", 4, 4, hSub);
        
	return TRUE;  // return TRUE unless you set the focus to a control
	              // EXCEPTION: OCX Property Pages should return FALSE
}
