...
CToolKitDlg::CToolKitDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CToolKitDlg::IDD, pParent)
{
...
void CToolKitDlg::InitTabCtrl()
{
    CString str;
    for (int i = IDS_STR_PROCESS; i < IDS_STR_TODO; i++)
    {
        str.LoadString(i);
        m_TabCtrl.InsertItem(i - IDS_STR_PROCESS, str);
    }
}

void CToolKitDlg::DoProcess()
{
    int i = IDS_STR_PROCESS - IDS_STR_PROCESS;
    m_pDlg[i] = new CProcessDlg;
    ...
    m_pDlg[i]->Create(IDD_DLG_PROCESS, &m_TabCtrl);
    m_pDlg[i]->MoveWindow(rc, TRUE);
    
    m_pDlg[i]->ShowWindow(SW_NORMAL);
}
