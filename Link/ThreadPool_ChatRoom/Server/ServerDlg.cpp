...
void CServerDlg::OnStartserver() 
{
    ...
    //采用异步选择模型
    int nRet = ::WSAAsyncSelect(m_MySocket.m_Socket,
                    GetSafeHwnd(),
                    WM_SOCKET,
                    FD_ACCEPT | FD_READ /*| FD_CLOSE*/);
    if (SOCKET_ERROR == nRet)
    {
        CMySocket::ShowErrorMessage();
    }
...
UINT WaitThread( LPVOID pParam )
{
    ...
    while (TRUE)
    {
        ...
        bRet = pDlg->m_MySocket.Accept(pClient);//already modifyed
        if (!bRet)
        {
            //err
        ...
        //每隔一定时间发送心跳包
        SOCKET sk = pClient->m_Socket;
        AfxBeginThread(HeartBeatThread, &sk);
...
LRESULT CServerDlg::OnNetEvents(WPARAM wParam, LPARAM lParam)
{
...
{
    //应该交由ThreadPool中的线程来处理，减小消息队列的处理压力
    CAcceptWorkItem *pAcceptItem = new CAcceptWorkItem(this);
    if (NULL == pAcceptItem)
    {
        return FALSE;
    }

    m_ThreadPool.AddWork(pAcceptItem);    
...
{
    //应该交由线程池进行处理，减小消息队列的压力
    CRecvWorkItem *pRecvItem = new CRecvWorkItem(this, wParam);
    if (NULL == pRecvItem)
    {
        return FALSE;
    }

    m_ThreadPool.AddWork(pRecvItem);
