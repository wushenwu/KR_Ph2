...
BOOL CClientDlg::OnInitDialog()
{
    ...
    //关于服务端地址的处理
    g_addrTO.sin_family = AF_INET; /*地址族*/
    g_addrTO.sin_port = htons(PORT);  //0由系统安排一个没有占用的端口
    g_addrTO.sin_addr.S_un.S_addr = inet_addr(HOST);
...
void CClientDlg::OnStartclient() 
{
    ...
    */
    CMyPacket::NotifyUDPInit("Hi, I'm going to talk with you",
                            m_MySocket.m_Socket,
                            (sockaddr *)&g_addrTO,
                            sizeof(sockaddr));
...
UINT ProcessServerThread( LPVOID pParam )
{
    ...
    while (TRUE)
    {
        ...
        bRet = CMySocket::RecvFrom(pSocket->m_Socket, 
                                    szBuf, 
                                    MAXBYTE, 
                                    0,
                                    (sockaddr *)&addrin,
...
void CClientDlg::OnStopclient() 
{
    ...
    sockaddr_in addrin;
    CString str;
    CMySocket::GetAddress(m_MySocket.m_Socket, &addrin, str);
...
BOOL CClientDlg::PreTranslateMessage(MSG* pMsg) 
{
        ...
        && VK_RETURN == pMsg->wParam)
    {
            ...
            && m_IsStart)
        {
            ...
            CMyPacket::NotifyChat(m_bBroadCast, 
                                    szLine,
                                    m_MySocket.m_Socket, 
                                    m_strReq.GetBuffer(0),
                                    FALSE,  //UDP
                                    (sockaddr *)&g_addrTO,
                                    sizeof(sockaddr_in)
                                    );
