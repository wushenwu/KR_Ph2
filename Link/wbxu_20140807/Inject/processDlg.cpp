...
{
    ProcessWnd& data = *(ProcessWnd*)lParam;
    DWORD dwPID = 0;
...
void CProcessDlg::OnDblclkListProcess(NMHDR* pNMHDR, LRESULT* pResult) 
{
    ...
    //同时对窗口标题进行设置
    UpdateData(TRUE);   //获取PID
    ASSERT(m_dwPID != 0);

    HWND hWnd = FindMainWindow(m_dwPID);
    if (NULL == hWnd)
    {
        return;
    }

    TCHAR szCaption[MAXBYTE] = {0};
    ::GetWindowText(hWnd, szCaption, MAXBYTE);
    SetDlgItemText(IDC_WNDNAME, szCaption);
...
void CProcessDlg::Say2Dll()
{
    ...
    //拷贝数据
    strcpy((char*)lpBuff, m_WndName.GetBuffer(0));
